...
resource "aws_iam_policy" "eks_cluster_autoscaler_policy" {
  ...
  policy = file("eks-cluster-autoscale.json")
}
...
###################################################################################################
# Apply the Tags to ASG created by EKS Cluster
###################################################################################################
...
resource "aws_autoscaling_group_tag" "tag1_eks_asg" {

  autoscaling_group_name = aws_eks_node_group.eksnode.resources[0].autoscaling_groups[0].name

  tag {
    key   = "k8s.io/cluster-autoscaler/${var.eks_cluster}-${var.env}"
    value = "owned"

    propagate_at_launch = false  ### Do you required to apply the tags to EC2 Instances launched as an integral part of ASG
  }

  depends_on = [aws_eks_node_group.eksnode]      #[time_sleep.wait_30_seconds]
}

resource "aws_autoscaling_group_tag" "tag2_eks_asg" {
